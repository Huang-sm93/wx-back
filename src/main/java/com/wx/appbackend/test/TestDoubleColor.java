...
public class TestDoubleColor {
    ...
    public static void main(String[] args) throws WriteException, IOException {
        ...
        System.out.println("mostRedList:" + mostRedList.get(0));
        System.out.println("mostRedList:" + mostRedList.get(0).subList(1, 8));
        System.out.println("mostRedList:" + mostRedList.get(0).subList(mostRedList.get(0).size()-12, mostRedList.get(0).size()));

        List<Integer> canSelectList = mostRedList.get(0);
        Random random = new Random();
        int count = canSelectList.size()/8;
        for (int i = 0; i < count; i++) {
            List<Integer> tempList = new ArrayList<>();
            for (int j = 0; j < 8; j++) {
                int index = random.nextInt(canSelectList.size());
                tempList.add(canSelectList.get(index));
                canSelectList.remove(index);
            }
            System.out.println("第"+i+"次：" + tempList);
        }
        System.out.println("剩余：" + canSelectList);
...
//            List<Integer> mostList = mostRedList.get(i).subList(1, 8);
//            List<Integer> lessList = mostRedList.get(i).subList(mostRedList.get(i).size()-12, mostRedList.get(i).size());
...
//            try {
//                book = Workbook.createWorkbook( new File("统计命中1.xls" ));
